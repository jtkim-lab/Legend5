...
//HUNJAE START
//1. Get ID Array
//2. Make Sentence by using ID-WORD Table
//3. input_string = THE SENTENCE
...
int sentence_processing(char * ID_array, int block_cnt, int double_flag){
	char input_string[MAXINPUT]="";
	char pico_string[MAXINPUT]="";
	char play_string[MAXINPUT]="";
	char amixer_string[MAXINPUT]="";
	int result=0;

	memset(input_string,0,MAXINPUT);

	for(i=0;i<block_cnt;i++){
		strcat(input_string,WordTable[ID_array[i]]);
		strcat(input_string," ");
	}
	...
	if (special_command(input_string, dict)) return 0;
	...
	if ((num_linkages == 0) && (!parse_options_get_batch_mode(opts))) {
		...
		//JT
		strcat(pico_string, "pico2wave -w input.wav '");
		strcat(pico_string, input_string);
		strcat(pico_string, "'");
	
		strcat(play_string, "aplay input.wav");
		strcat(amixer_string, "amixer cset numid=3 1 100%");

		system(pico_string);
		system(amixer_string);
		system(play_string);
